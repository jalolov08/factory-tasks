...
export default function TaskOrder({ route, navigation }: TaskOrderScreenProps) {
  ...
  if (error || !order)
    return <Text style={styles.errorText}>Не удалось загрузить заказ.</Text>;
        ...
        <Card style={styles.card}>
          <Card.Content>
            ...
            <Field
              ...
              value={`${order.priceDiscounted} ${order.currency}`}
            />
            <Field label="Скидка" value={`${order.discount}%`} />
            <Field
              label="Сумма"
              value={`${order.totalCost} ${order.currency}`}
            />
            <Divider style={styles.divider} />
            ...
            <Field
              label="Выдано"
              value={`${order.given} ${order.unitOfMeasurement}`}
            />
            <Divider style={styles.divider} />
